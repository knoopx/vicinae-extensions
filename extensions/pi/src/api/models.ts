...
/**
 * Retrieves cached models from Cache
 */
export async function getCachedModels(): Promise<AvailableModel[] | null> {
  try {
    const cached = await Cache.getItem<string>(MODELS_CACHE_KEY);
    if (cached) {
...
/**
 * Caches models to Cache
 */
export async function cacheModels(models: AvailableModel[]): Promise<void> {
  try {
    await Cache.setItem(MODELS_CACHE_KEY, JSON.stringify(models));
  } catch (error) {
